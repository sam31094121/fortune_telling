...
exports.ICHING_RITUAL_STEPS = exports.THREE_CORE_ENGINE = void 0;
exports.runBaziLayer = runBaziLayer;
exports.runZiweiLayer = runZiweiLayer;
exports.isZiweiCertified = isZiweiCertified;
exports.pillarsWellFormedOf = pillarsWellFormedOf;
exports.runIChingLayer = runIChingLayer;
exports.computeThreeCore = computeThreeCore;
exports.assertThreeCoreConsistent = assertThreeCoreConsistent;
const engine_1 = require("./bazi/engine");
const ziwei_sanfang_engine_1 = require("./ziwei-sanfang-engine");
const iching_engine_1 = require("./iching-engine");
const iching_psychology_1 = require("./iching-psychology");
exports.THREE_CORE_ENGINE = {
...
function isKnownHour(index) {
    return typeof index === 'number' && Number.isInteger(index) && index >= 0 && index <= 11;
}
function inputHourIndex(input) {
    if (!input.birthTime)
        return isKnownHour(input.hourBranchIndex) ? input.hourBranchIndex : null;
    if (!/^([01]\d|2[0-3]):[0-5]\d$/.test(input.birthTime))
        throw new Error('THREE_CORE_INVALID_EXACT_TIME');
    const index = Math.floor((Number(input.birthTime.slice(0, 2)) + 1) / 2) % 12;
    if (input.hourBranchIndex !== undefined && input.hourBranchIndex !== index)
        throw new Error('THREE_CORE_TIME_INPUT_MISMATCH');
    return index;
}
...
function elementBalanceOf(pillars) {
 ...
 */
function runBaziLayer(input) {
    const hourIndex = inputHourIndex(input);
    const hourKnown = hourIndex !== null;
    const hourBranch = hourKnown ? engine_1.BRANCHES[hourIndex] : null;
    const core = (0, engine_1.createBaziCore)({
        gender: input.gender,
...
function runZiweiLayer(input, existingCore) {
    const hourIndex = inputHourIndex(input);
    if (hourIndex === null) {
        return {
    ...
    return {
        status: 'READY',
        analysis: (0, ziwei_sanfang_engine_1.calculateZiweiSanFang)({
            birthDate: input.birthDate,
 ...
 */
function isZiweiCertified(ziwei) {
...
function ziweiMatchesBazi(bazi, ziwei) {
    return ziwei.status === 'READY' && bazi.hour !== null
        && ['year', 'month', 'day', 'hour'].every(key => ziwei.analysis.bazi[key] === bazi[key]);
}
/** 四柱格式是否成立（兩字、天干地支都認得）。 */
function pillarsWellFormedOf(bazi) {
...
*/
function runIChingLayer(params) {
    ...
    */
    ...
    const coreMatches = ['year', 'month', 'day', 'hour'].every(key => {
        const pillar = core.pillars[key];
        return (pillar === 'UNKNOWN' ? null : pillar.ganZhi) === bazi[key];
    });
    const normalizeDate = (date) => date.trim().split('-').map(Number).join('-');
    const inputMatches = normalizeDate(input.birthDate) === normalizeDate(core.calendar.solarDate)
        ...
        && hourIndex !== null && bazi.hourBranch === engine_1.BRANCHES[hourIndex];
    ...
    const ritualSteps = exports.ICHING_RITUAL_STEPS.map((step) => ({
        ...step,
    ...
    */
    ...
    const reading = (0, iching_engine_1.castHexagramCertified)(certificate, input.birthDate, hourIndex);
    return { status: 'READY', reading, patternName: (0, iching_psychology_1.patternNameOf)(reading), ritual, certificate };
}
function computeThreeCore(input) {
    const hourIndex = inputHourIndex(input);
    const hourKnown = hourIndex !== null;
    ...
    return {
        engine: exports.THREE_CORE_ENGINE,
 ...
 */
function assertThreeCoreConsistent(result) {
    ...
    const readingMatches = result.iching.status !== 'READY' || (result.ziwei.status === 'READY' && isZiweiCertified(result.ziwei) && currentPillarsMatch
