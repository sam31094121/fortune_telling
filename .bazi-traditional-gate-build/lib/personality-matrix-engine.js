...
exports.PersonalityMatrixEngine = void 0;
const music_personality_db_1 = require("./music-personality-db");
class PersonalityMatrixEngine {
    /**
     * 從天模型提取人格參數（35%）
     */
    static extractSkyModel(birthDate, zodiacSign) {
        const matrix = {
            emotion: 0,
            logic: 0,
            social: 0,
        ...
        // 星座基礎人格
        const zodiacBase = music_personality_db_1.ZodiacPersonalityMap[zodiacSign] || {
            emotion: 70,
            logic: 70,
            social: 70,
            leadership: 70,
        ...
        const birthdayAdjust = music_personality_db_1.BirthdayPersonalityMap[dayOfMonth] || {};
        // 月份微調（季節性）
     ...
     * 從地模型提取人格參數（35%）
     */
    static extractEarthModel(bloodType, voiceCharacteristics = []) {
        const matrix = {
            emotion: 0,
            logic: 0,
            social: 0,
        ...
        // 血型人格基礎
        const bloodTypeBase = music_personality_db_1.BloodTypeMap[bloodType] || {
            emotion: 70,
            logic: 70,
            social: 70,
            leadership: 70,
        ...
        for (const characteristic of voiceCharacteristics) {
            const charAdjust = music_personality_db_1.VoiceCharacteristicMap[characteristic] || {};
     ...
     */
    static extractHumanModel(firstName, gender, lastName) {
        const matrix = {
            emotion: 70,
            logic: 70,
            social: 70,
        ...
        const nameAdjust = music_personality_db_1.NameSemanticMap[strokeCount] || {};
        // 聲調偏好（漢語聲調或拼音）
        const toneAdjust = this.getToneAdjustment(firstName);
        // 性別（男女）修正：併入人 30%，不再算在地層
        const genderVoice = music_personality_db_1.GenderVoiceMap[gender];
     ...
     */
    static generatePersonalityMatrix(input, destinyAdjust, shichenAdjust) {
     ...
     * 輔助方法：估計漢字筆畫（簡化版）
     */
    static estimateStrokeCount(char) {
     ...
     * 輔助方法：聲調調整
     */
    static getToneAdjustment(firstName) {
        // 簡化邏輯：根據首字母判斷發音氣質
     ...
     * 輔助方法：季節調整
     */
    static getSeasonAdjustment(month) {
        const seasonMap = {
...
exports.PersonalityMatrixEngine = PersonalityMatrixEngine;
