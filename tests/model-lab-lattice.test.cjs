...
const { entrancePoint, portalProjection, ENTRANCE_SCALE } = moduleBox.exports;
close(ENTRANCE_SCALE * (1 - BEAM), .564);
close(ENTRANCE_SCALE * (1 + BEAM), .636);
for (let axis = 0; axis < 3; axis++) {
  const p = [.5, .5, .2], q = [...p]; q[axis] += 1;
  close(Math.hypot(...entrancePoint(q).map((v, i) => v - entrancePoint(p)[i])), .6);
}
for (const eye of [2.7, 5.1, Math.SQRT1_2 + .072]) {
...
close(entry[0], 0); close(entry[1], Math.SQRT1_2 + .072); close(entry[2], 0);
...
close(sweepFraction([.5, .1, .5], [2, 0, 0]), .175);
const stopped = moveSafely([.5, .1, .5], [2, 0, 0]);
assert.ok(stopped[0] < .85 && stopped[0] > .8499);
assert.equal(sweepFraction([0, 0, .5], [.1, 0, 0]), 0); // initial overlap safely blocked
assert.equal(sweepFraction([.15, .1, .5], [.5, 0, 0]), 1); // leave a touching face
close(sweepFraction([.15, .1, .5], [-.1, 0, 0]), 0); // cannot enter same face
