...
(async () => {
  ...
  try {
    ...
    assert.match(visual.animation, /skillCharge/, 'Card movement is attached to the performed attack');
    await page.waitForTimeout(3900);
    ...
    const home = page.getByRole('link',{name:'回首頁',exact:true});
    assert.equal(await home.getAttribute('href'),'/');
    const homeBox=await home.boundingBox();assert.ok(homeBox.height>=44&&homeBox.y+homeBox.height<=844,'Result has a visible touch-sized homepage exit');
