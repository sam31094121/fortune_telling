...
if (!taijiSystem.includes("level01StrikeRoutePoints('N')") || !taijiSystem.includes("level01StrikeRoutePoints('S')") || !taijiSystem.includes("level01StrikeRoutePoints('E')") || !taijiSystem.includes("level01StrikeRoutePoints('W')")) throw new Error('all four cardinal photon-particle cores must expose one shared strike route');
if (!taijiSystem.includes('Level01SpatialLightning({ active, origin, variant, strikeId, lowPower') || !taijiSystem.includes('group.rotation.set(0, 0, 0)') || taijiSystem.includes('origin={lightningOrigin} poseRef=')) throw new Error('the card-space lightning field must remain fixed while the Taiji rotates');
...
if (!styles.includes('.chaseField .chaseLight { left: 50%; top: 90%') || !styles.includes('.chaseField .chaseCounterLight { left: 3%; top: 50%') || !styles.includes('.chaseField .chasePhotonNorth { left: 50%; top: 3%') || !styles.includes('.chaseField .chaseParticleEast { left: 97%; top: 50%')) throw new Error('the four level 01 cores must stay at north, east, south, and west with the south core visually grounded');
...
if (!styles.includes('SHADOW_PHOTON_DEPTH_LOCK') || !styles.includes('.chaseField .chaseLight { left: 50%; top: 90%;')) throw new Error('the south photon must share the existing ground-shadow depth instead of floating behind it');
if (!taijiSystem.includes('LEVEL_01_JADE_GLAZE') || !taijiSystem.includes('if (layer === 1)') || !taijiSystem.includes('ballMat.clearcoat = 0.72') || !taijiSystem.includes('ballMat.envMapIntensity = 0.94')) throw new Error('level 01 must keep its softened black-jade reflection without changing later layers');
...
if (!audio.includes('const ORIGIN_PAN') || !audio.includes('const impactAt = now + LEVEL01_STRIKE_IMPACT_SECONDS') || !taijiSystem.includes('LEVEL01_STRIKE_IMPACT_SECONDS') || !audio.includes('panner.pan.linearRampToValueAtTime(0')) throw new Error('lightning audio and the 3D route must share one selected-core impact instant');
...
if (!audio.includes('const THUNDER_SOURCES') || !audio.includes("'/audio/taiji/lightning-strike.mp3'") || !audio.includes("'/audio/taiji/dry-thunder.mp3'") || !audio.includes("'/audio/taiji/loud-thunder.mp3'") || !audio.includes("'/audio/taiji/peals-of-thunder.mp3'") || !audio.includes("'/audio/taiji/cc0-cannon-fire.ogg'") || !audio.includes("'/audio/taiji/cc0-cannon-hit.ogg'") || !audio.includes("'/audio/taiji/cc0-cannon-hit-cannon.ogg'") || !audio.includes('this.lightningVariantIndex += 1')) throw new Error('lightning must rotate through the original four and later licensed impact recordings');
if (audio.includes('Math.random(') || audio.includes('createOscillator(')) throw new Error('level 01 thunder must not synthesize noise, tones, or a repeating hum');
if (!audio.includes('primeFromUserGesture') || !audio.includes('this.context.createBuffer(1, 1, this.context.sampleRate)')) throw new Error('the first real point gesture must prime its silent audio graph before the original thunder strike');
