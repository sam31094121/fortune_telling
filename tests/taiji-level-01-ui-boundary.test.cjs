...
if (!overlay.includes('styles.chasePhotonTop') || !overlay.includes('styles.chaseParticleTop') || !styles.includes('第一層四核心循環')) throw new Error('level 01 must keep the proportional four-core photon-particle circulation');
...
if (!audio.includes('const fourCoreVoices = [') || !audio.includes('const impactAt = now + 0.17') || !audio.includes('panner.pan.linearRampToValueAtTime(0, impactAt)')) throw new Error('lightning audio must circulate through four photon-particle cores before converging on impact');
if (!audio.includes('const thunderShapes = [') || !audio.includes('this.lightningVariantIndex += 1') || !audio.includes("growl.type = 'sawtooth'")) throw new Error('lightning must rotate deterministic natural thunder shapes with a bounded angry growl');
