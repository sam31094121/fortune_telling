...
export default function BeastDuelRitual({ player, opponent, timeline, replay, pairs, onComplete, onCancel }: Props) {
  ...
  /** 有分層傀儡 manifest 時用引擎；否則 null → 退回 chargeVideoFor。 */
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setChargePuppetPoolId(null);
    const vids = chargeVideoFor(id);
    setChargeVideoSrc(vids.webm); // fallback 先備好；有 puppet 時 UI 優先引擎
    void (async () => {
      ...
      <div className={`${styles.center} ${phase === 'clash' || pairClash !== null ? styles.clash : ''}`} role="status" aria-live="polite" style={{ position: 'relative' }}>
        {pairClash !== null && player[pairClash] && opponent?.[pairClash] && (
          <>
            {chargeSkillLabel ? (
              <div
                style={{ position: 'absolute', top: 12, left: '50%', transform: 'translateX(-50%)', zIndex: 6, padding: '4px 10px', borderRadius: 999, background: 'rgba(0,0,0,.55)', color: '#fff', fontSize: 13, letterSpacing: '.08em' }}
                aria-live="polite"
              >
                {chargeSkillLabel}
              </div>
            ) : null}
            {/* 影片當底層 fallback；引擎傀儡蓋在上面。若 puppet 404 回 null，影片仍可見。 */}
            {chargeVideoSrc ? (
              <video
                key={chargeVideoSrc + String(pairClash)}
                autoPlay
                playsInline
                muted={Boolean(chargePuppetPoolId)}
            ...
            {chargePuppetPoolId ? (
              <BeastLayeredPuppet
                ...
                onDone={() => { /* 6s 結束；儀式本身仍由 pairClash timer 收尾 */ }}
              />
            ) : null}
            <BeastClash3D
