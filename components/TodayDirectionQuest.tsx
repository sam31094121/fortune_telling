...
function readQuestHistory(): QuestHistory {
  try {
    const raw = window.localStorage.getItem(QUEST_HISTORY_KEY);
    if (!raw) return { streak: 0, lastCompletedDate: '', totalDays: 0 };
    const parsed = JSON.parse(raw) as Partial<QuestHistory>;
    ...
    return {
      ...
      totalDays: Number.isSafeInteger(totalDays) && totalDays > 0 ? totalDays : 0,
    };
  } catch {
    return { streak: 0, lastCompletedDate: '', totalDays: 0 };
  }
}
  ...
  return (
    ...
    >
      ...
      <div className={styles.content}>
        ...
        {(stage === 'tension' || stage === 'action' || stage === 'area') && returnNote && (
          <p className={styles.returnNote} aria-live="polite">{returnNote}</p>
        )}
        ...
        {stage === 'intro' && (
          <div className={styles.intro}>
            <div className={styles.introCopy}>
              ...
              {streakDays > 1 && (
                <p className={styles.streakChip} aria-label={`已連續 ${streakDays} 天`}>
                  已連續 {streakDays} 天
                </p>
              )}
        ...
        {stage === 'checkin' && (
          <div className={styles.step}>
            ...
            {returnNote && <p className={styles.returnNote} aria-live="polite">{returnNote}</p>}
            {streakDays > 1 && (
              <p className={styles.streakChip} aria-label={`已連續 ${streakDays} 天`}>已連續 {streakDays} 天</p>
            )}
