...
const WEIGHT_DISPLAY = [
  { label: '天', value: 35, desc: '生日建立人格骨架' },
  { label: '地', value: 35, desc: '血型補充行為模式' },
  { label: '人', value: 30, desc: '姓名解鎖個體差異' },
];
...
export default function ResultDisplay({
  ...
  const guidanceBlocks = isUnlocked
    ? [
        { title: '天｜人格骨架', body: vipResult.skeleton_summary, tone: 'sky' },
        { title: '地｜行為模式', body: vipResult.behavior_summary, tone: 'earth' },
        { title: '人｜個體差異', body: vipResult.individuality_summary, tone: 'human' },
        { title: '善念提醒', body: vipResult.wisdom_perspective, tone: 'earth' },
      ]
    : [
        { title: '天｜人格骨架', body: previewResult.skeleton_summary, tone: 'sky' },
        { title: '地｜行為模式', body: previewResult.behavior_summary, tone: 'earth' },
        { title: '天地摘要', body: previewResult.preview_summary, tone: 'human' },
      ];
  ...
  async function handleUnlock() {
    ...
    if (trimmed.length < 2) {
      setLocalError('姓名至少需要 2 個字，才能啟動 VIP 解碼。');
      return;
    }
    ...
    <div className="space-y-6">
      ...
      {!isUnlocked ? (
        <div className="fortune-card human-card p-6 sm:p-8">
          <div className="space-y-3">
            <p className="text-xs uppercase tracking-[0.35em] text-pink-300">VIP 人層解碼</p>
            <h3 className="font-serif text-3xl text-[color:var(--text-main)]">輸入姓名，解鎖剩餘 30%</h3>
            <p className="max-w-2xl text-sm leading-8 text-[color:var(--text-sub)]">
              天地已完成免費運算。姓名會作為最後的人層校正器，補充個體差異、財富動機、感情依附與人生優勢。
            </p>
          </div>
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            ...
            >
              {isUnlocking ? '姓名能量解碼中…' : '啟動 VIP 人層解碼'}
            </button>
            <button
              type="button"
              onClick={onReset}
        ...
        <div className="vip-gold-card rounded-[24px] p-6 sm:p-8">
          ...
          <p className="mt-4 text-sm leading-8 text-[color:var(--text-main)]">
            天地人會幫你看見自己的底層傾向，但真正能讓人生變好的，還是你每天的選擇。
