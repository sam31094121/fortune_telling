...
import { ENTRANCE_SCALE, ENTRANCE_Y, portalProjection } from './latticeMath';
...
function EntranceScene({ settings, entering, onEnter, requestEnter }: {
  ...
  useFrame(({ camera, size }, dt) => {
    ...
    eyeY.current = THREE.MathUtils.lerp(start, ENTRANCE_Y + .12 * ENTRANCE_SCALE, ease);
  ...
  return <>
    ...
    <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, ENTRANCE_Y, 0]} onClick={requestEnter}>
...
export default function LatticeEntrance({ settings, onEnter, onExit }: {
  ...
  return <section className={styles.interior} aria-label="太極鏤空方格入口預覽">
    ...
    <div className={styles.controls}>
      ...
      <button disabled={!open || entering} onClick={() => setEntering(true)}>{!open ? '通道被前／後圓片封住' : entering ? '正在穿過方口…' : '穿過方口 · 進入延伸空間'}</button>
