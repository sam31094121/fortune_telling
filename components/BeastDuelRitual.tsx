      ...
      <div className={`${styles.center} ${phase === 'clash' || pairClash !== null ? styles.clash : ''} ${fusionBeat ? styles.fusionClash : ''}`} role="status" aria-live="polite" style={{ position: 'relative' }}>
        ...
        */}
        ...
        {!clashActive && <div className={styles.waitingCards} aria-hidden="true">
          {(() => {
            const oCard = opponent?.[previewIndex];
            const pCard = player?.[previewIndex];
            return <>
              ...
              <div className={styles.waitingPlayer} key="player">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={pCard && isRevealed('player', previewIndex) ? pCard.thumbnail : '/beast-game/card-back.webp'} alt="" />
                <span>客戶（你）・{POSITIONS[previewIndex]}</span>
              </div>
            </>;
          })()}
        </div>}
