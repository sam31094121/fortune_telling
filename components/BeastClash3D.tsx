...
export default function BeastClash3D({
  ...
  /* Static fallback mirrors battle seating: computer top, customer bottom. */
  const staticBeasts = active ? <div data-beast-static-fallback className="absolute inset-0 flex flex-col items-center justify-between gap-2 p-6" aria-label="神獸本體靜態展示">
    ...
    {playerSpirit ? (
      // eslint-disable-next-line @next/next/no-img-element
      <img src={playerSpirit} alt="客戶神獸本體" className="h-[42%] max-h-full w-full min-h-0 object-contain" />
    ) : <span className="h-[42%]" />}
  </div> : null;
  ...
  return (
    ...
    >
      ...
      >
        ...
        */}
        <Arena />
        {/* Vertical seating: player lower (customer bottom), opponent higher (computer top). */}
