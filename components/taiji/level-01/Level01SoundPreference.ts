import type { TaijiSoundVariant } from '@/lib/taiji/experience-types';

/**
 * 匿名聲音偏好學習：禁止事先寫死「大數據最喜歡」。
 * 每支裝置先被隨機分到 4 個候選聲音之一，累積真實互動資料，
 * 樣本數不夠前絕不宣布贏家；夠了才用完成率/靜音率/回放率/下一步轉換率評出目前表現最好的一個。
 * 本機 localStorage 是快取＋離線後備；真正的樣本累積是 /api/taiji-sound-preference
 * （Supabase 聚合表，跨使用者），事件用背景 fetch 送出，絕不擋住儀式節奏。
 * 不上傳麥克風、不收裝置識別碼、不收敏感資料，只累積「哪個候選音色＋哪種事件」的次數。
 */

export const TAIJI_SOUND_VARIANTS: readonly TaijiSoundVariant[] = ['SOFT_WOOD', 'WARM_BELL', 'AIR_CHIME', 'LOW_RESONANCE'];

const STORAGE_KEY = 'tdh_taiji_sound_preference_v1';
// 每個候選聲音至少要有這麼多次「被分配到」的紀錄，才允許進入贏家評估；
// 資料不夠寧可保持隨機分配，也不假稱有結論。
const MIN_SAMPLE_SIZE = 40;
const SYNC_ENDPOINT = '/api/taiji-sound-preference';

export interface Level01SoundVariantStats {
  assigned: number;
  completedInteraction: number;
  mutedImmediately: number;
  replayed: number;
  nextStepCompleted: number;
}

interface Level01SoundPreferenceStore {
  deviceVariant: TaijiSoundVariant | null;
  stats: Record<TaijiSoundVariant, Level01SoundVariantStats>;
  winner: TaijiSoundVariant | null;
}

type ServerEventField = 'assigned' | 'completed' | 'muted_immediately' | 'replayed' | 'next_step';

type ServerStatsRow = {
  variant?: unknown;
  assignedCount?: unknown;
  completedCount?: unknown;
  mutedImmediatelyCount?: unknown;
  replayedCount?: unknown;
  nextStepCount?: unknown;
};

function isVariant(value: unknown): value is TaijiSoundVariant {
  return typeof value === 'string' && (TAIJI_SOUND_VARIANTS as readonly string[]).includes(value);
}

function emptyStats(): Level01SoundVariantStats {
  return { assigned: 0, completedInteraction: 0, mutedImmediately: 0, replayed: 0, nextStepCompleted: 0 };
}

function emptyStore(): Level01SoundPreferenceStore {
  const stats = {} as Record<TaijiSoundVariant, Level01SoundVariantStats>;
  for (const variant of TAIJI_SOUND_VARIANTS) stats[variant] = emptyStats();
  return { deviceVariant: null, stats, winner: null };
}

function readStore(): Level01SoundPreferenceStore {
  if (typeof window === 'undefined') return emptyStore();
  try {
    const raw = window.localStorage.getItem(STORAGE_KEY);
    if (!raw) return emptyStore();
    const parsed = JSON.parse(raw) as Partial<Level01SoundPreferenceStore> | null;
    const base = emptyStore();
    if (!parsed || typeof parsed !== 'object') return base;
    for (const variant of TAIJI_SOUND_VARIANTS) {
      const incoming = parsed.stats?.[variant];
      if (incoming) base.stats[variant] = { ...emptyStats(), ...incoming };
    }
    return {
      deviceVariant: isVariant(parsed.deviceVariant) ? parsed.deviceVariant : null,
      stats: base.stats,
      winner: isVariant(parsed.winner) ? parsed.winner : null,
    };
  } catch {
    return emptyStore();
  }
}

function writeStore(store: Level01SoundPreferenceStore) {
  if (typeof window === 'undefined') return;
  try {
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(store));
  } catch {
    // 偏好學習是加分功能；儲存被擋掉時，儀式聲音仍然正常播放，只是這次不記錄。
  }
}

/** 樣本數不夠任何一個候選就不評，夠了才用完成率/靜音率/回放率/下一步轉換率評分。 */
function evaluateWinner(entries: Array<{ variant: TaijiSoundVariant; assigned: number; completed: number; muted: number; replayed: number; nextStep: number }>): TaijiSoundVariant | null {
  if (entries.length < TAIJI_SOUND_VARIANTS.length) return null;
  if (entries.some((entry) => entry.assigned < MIN_SAMPLE_SIZE)) return null;
  const scored = entries.map((entry) => {
    const completionRate = entry.completed / entry.assigned;
    const muteRate = entry.muted / entry.assigned;
    const replayRate = entry.replayed / entry.assigned;
    const nextStepRate = entry.nextStep / entry.assigned;
    const score = completionRate * 0.4 + nextStepRate * 0.35 + replayRate * 0.15 - muteRate * 0.4;
    return { variant: entry.variant, score };
  });
  scored.sort((a, b) => b.score - a.score);
  return scored[0].variant;
}

export class Level01SoundPreferenceEngine {
  private store: Level01SoundPreferenceStore;

  constructor() {
    this.store = readStore();
  }

  /** 已經有贏家就大家聽贏家；沒有的話維持這支裝置原本分配到的那一個，不要每次重洗。 */
  resolveVariant(): TaijiSoundVariant {
    if (this.store.winner) return this.store.winner;
    if (!this.store.deviceVariant) {
      const index = Math.floor(Math.random() * TAIJI_SOUND_VARIANTS.length);
      this.store.deviceVariant = TAIJI_SOUND_VARIANTS[index];
      this.store.stats[this.store.deviceVariant].assigned += 1;
      writeStore(this.store);
      this.syncToServer(this.store.deviceVariant, 'assigned');
    }
    return this.store.deviceVariant;
  }

  recordEvent(event: {
    variant: TaijiSoundVariant;
    completedInteraction?: boolean;
    mutedImmediately?: boolean;
    replayed?: boolean;
    nextStepCompleted?: boolean;
  }) {
    const stats = this.store.stats[event.variant];
    if (!stats) return;
    if (event.completedInteraction) { stats.completedInteraction += 1; this.syncToServer(event.variant, 'completed'); }
    if (event.mutedImmediately) { stats.mutedImmediately += 1; this.syncToServer(event.variant, 'muted_immediately'); }
    if (event.replayed) { stats.replayed += 1; this.syncToServer(event.variant, 'replayed'); }
    if (event.nextStepCompleted) { stats.nextStepCompleted += 1; this.syncToServer(event.variant, 'next_step'); }
    writeStore(this.store);
    this.reevaluate();
  }

  /**
   * 背景拉一次跨使用者聚合統計，看看真實贏家有沒有出來；不影響本次已經在播的聲音。
   * 呼叫端請 fire-and-forget，不必 await——這是給下一次造訪用的，不是本次儀式的必要條件。
   */
  async refreshWinnerFromServer() {
    if (this.store.winner || typeof fetch === 'undefined') return;
    try {
      const response = await fetch(SYNC_ENDPOINT, { cache: 'no-store' });
      if (!response.ok) return;
      const json = await response.json() as { ok?: boolean; stats?: ServerStatsRow[] };
      if (!json.ok || !Array.isArray(json.stats)) return;
      const entries = json.stats
        .filter((row): row is Required<ServerStatsRow> & { variant: TaijiSoundVariant } => isVariant(row.variant))
        .map((row) => ({
          variant: row.variant,
          assigned: Number(row.assignedCount) || 0,
          completed: Number(row.completedCount) || 0,
          muted: Number(row.mutedImmediatelyCount) || 0,
          replayed: Number(row.replayedCount) || 0,
          nextStep: Number(row.nextStepCount) || 0,
        }));
      const winner = evaluateWinner(entries);
      if (winner) {
        this.store.winner = winner;
        writeStore(this.store);
      }
    } catch {
      // 純加分功能；拉不到聚合資料就繼續用這支裝置本地的分配，下次再試。
    }
  }

  private reevaluate() {
    if (this.store.winner) return;
    const entries = TAIJI_SOUND_VARIANTS.map((variant) => {
      const stats = this.store.stats[variant];
      return { variant, assigned: stats.assigned, completed: stats.completedInteraction, muted: stats.mutedImmediately, replayed: stats.replayed, nextStep: stats.nextStepCompleted };
    });
    const winner = evaluateWinner(entries);
    if (winner) {
      this.store.winner = winner;
      writeStore(this.store);
    }
  }

  private syncToServer(variant: TaijiSoundVariant, field: ServerEventField) {
    if (typeof fetch === 'undefined') return;
    try {
      void fetch(SYNC_ENDPOINT, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ variant, field }),
        keepalive: true,
      }).catch(() => undefined);
    } catch {
      // 純加分功能；送不出去不影響本地紀錄與儀式播放。
    }
  }

  snapshot(): Level01SoundPreferenceStore {
    return { deviceVariant: this.store.deviceVariant, stats: { ...this.store.stats }, winner: this.store.winner };
  }
}
