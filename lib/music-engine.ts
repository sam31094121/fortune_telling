...
const DIMENSION_KEYS: DimensionKey[] = [
  'emotion_sensitivity', 'logic', 'social_need', 'leadership',
  'risk_tendency', 'execution', 'creativity', 'empathy',
  'control', 'security_need', 'wealth_motivation', 'attachment',
];
...
function deriveSoundProfile(scores: DimensionScores): SoundProfile {
  ...
  const tempoRaw = scores.execution * 0.5 + scores.risk_tendency * 0.5;
  const intensityRaw = scores.leadership * 0.6 + scores.execution * 0.4;
  const emotionRaw =
    scores.emotion_sensitivity * 0.5 + scores.empathy * 0.3 + scores.attachment * 0.2;
  const structureRaw = scores.logic * 0.6 + scores.control * 0.4;

  return {
    tempo: tempoRaw >= 65 ? '快節奏' : tempoRaw >= 40 ? '適中' : '慢節奏',
    intensity: intensityRaw >= 65 ? '高張力' : intensityRaw >= 40 ? '平衡' : '輕柔',
    emotionDepth: emotionRaw >= 65 ? '情感濃郁' : emotionRaw >= 40 ? '情感適中' : '清爽理性',
    structure: structureRaw >= 65 ? '結構複雜' : structureRaw >= 40 ? '層次分明' : '簡潔直覺',
  };
}
