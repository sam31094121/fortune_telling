...
const CONTROL_CENTER_SECTIONS: ControlCenterSection[] = [
  ...
  },
  {
    id: 'rule_center',
    title: 'Rule Center',
    purpose: '統一管理命理規則、易經規則、商業規則與禁止事項。',
    status: 'ready',
    owner: 'Platform Control Center',
  ...
  },
  {
    id: 'ai_center',
    title: 'AI Center',
    purpose: '統一管理 易經 Core、Prompt、Model、Version 與禁止重算清單。',
    status: 'ready',
    owner: 'Single AI Core',
    lockedRule: '所有 易經文字整理與陪伴內容必須走同一個 易經 Core，不得每張卡片各自建立第二套 易經。',
  },
  {
    id: 'copywriting_center',
    title: '易經文案統一優化中心',
    purpose: '統一管理天地人和 易經的語氣、禁止詞、行動規則與陪伴語言。',
    status: 'ready',
    owner: 'AI Copywriting Style Center',
    lockedRule: '所有 易經文案必須清楚、直接、有方向、有行動，禁止模稜兩可。',
  },
  {
    id: 'growth_center',
    title: 'Growth Center',
...
const RULE_CENTER = [
  ...
  { id: 'single_ai_core', title: '唯一 易經 Core', rule: PLATFORM_AI_CORE_POLICY.rule },
  { id: 'ai_copywriting_style', title: '唯一 易經語言規範', rule: '所有 易經文案共用 易經文案統一優化中心，必須清楚、直接、有方向、有行動。' },
  { id: 'ai_action_guidance', title: '唯一 易經行動引導規範', rule: '每一次 易經卜卦分析完成後，必須回答目前判斷、現在最重要、下一步、改善方向。' },
  { id: 'ai_follow_up_system', title: '唯一 易經補強追蹤規範', rule: 'Follow-Up 只能追蹤補強進度、行動任務與本週提醒，禁止一般聊天與私生活提問。' },
...
const GROWTH_CONTROL = {
  ...
  cadence: 'weekly',
  weeklyItems: ['本週一句提醒', '本週第一補強元素', '本週唯一能量色', '本週一件行動任務', '易經補強追蹤', '本週一句成功人士公開名言'],
  monthlyItems: ['本月陪伴主題', '本月四週節奏'],
  rule: 'Growth Center 只能由 易經 Integration Layer 提供資料，不得直接重新呼叫六張命理分析。',
};

const AI_CENTER = {
  version: 'single_ai_core_v1',
  policy: PLATFORM_AI_CORE_POLICY,
  modelStrategy: '由環境與 易經 Core 統一管理，頁面不得各自指定第二套模型。',
...
export function buildPlatformControlCenter(now = new Date()) {
  ...
  const systemMonitor = [
    ...monitor,
    {
      ...
      detail: copywritingAudit.ok ? '易經文案中心規則通過禁止詞稽核。' : `偵測到 ${copywritingAudit.violationCount} 個禁止詞，需要調整。`,
    },
    {
  ...
  return {
    ...
    data: {
      ...
      masterFinalOptimize,
      title: '天地人和 易經 Platform Control Center',
