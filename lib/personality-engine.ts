...
function getTraitPhrase(key: DimensionKey, score: number) {
  ...
  const map: Record<DimensionKey, Record<string, string>> = {
    emotion: { 高: '情緒感知細膩，對氣氛變化非常敏銳', 中: '能感受到情緒波動，也保有自我緩衝', 低: '情緒相對穩定，不容易被外界波動帶動' },
    logic: { 高: '思考偏理性，做判斷時重視邏輯與證據', 中: '理性與直覺並用，能彈性看待問題', 低: '更依賴感受與直覺做決策' },
    social: { 高: '需要互動交流，從人際連結中獲得能量', 中: '能獨處也能社交，節奏較平衡', 低: '偏好私人空間，社交選擇較謹慎' },
    leadership: { 高: '容易主動帶方向，面對局勢有掌控感', 中: '在需要時願意承擔責任與帶頭', 低: '更擅長觀察與配合' },
    risk: { 高: '面對未知較敢嘗試，行動帶有突破性', 中: '會評估風險後再決定是否前進', 低: '偏好穩定節奏，避免不必要波動' },
    execution: { 高: '落地能力強，想法能快速轉成行動', 中: '執行節奏穩健，能按步完成目標', 低: '需要更強動機才容易啟動' },
    creativity: { 高: '想像力活躍，能提出新視角與新做法', 中: '兼具實用與創意，能靈活調整', 低: '偏好成熟方法，較少主動顛覆模式' },
    empathy: { 高: '容易感同身受，能接住他人情緒', 中: '具備理解他人的能力，也能保有界線', 低: '較重視事實與效率，不先從情緒切入' },
    control: { 高: '重視掌握度，習慣先規劃再行動', 中: '需要一定秩序，也能接受彈性', 低: '傾向順勢而行，不喜歡過度框住自己' },
    security: { 高: '非常重視安全感與可預期性', 中: '需要基本穩定，也接受適度變化', 低: '對不確定性的容忍度較高' },
    wealth: { 高: '對成果與資源累積有明確動機', 中: '重視實際回報，也在意生活平衡', 低: '較少被外在報酬驅動，更重視感受與意義' },
    attachment: { 高: '感情投入深，建立連結後很在意關係品質', 中: '能投入感情，也保留自我節奏', 低: '情感表達較克制，需要時間建立依附感' },
  };
...
export function enrichAnalysis(result: AnalysisResult): AnalysisResult {
  ...
  const skeleton_summary = result.ai_skeleton_summary?.trim()
  ...
  const behavior_summary = result.ai_behavior_summary?.trim()
    || (bloodEffects.length
      ? `血型在不推翻生日骨架的前提下，主要補強 ${bloodEffects.map((item) => `${item.label}${describeAdjustment(result.blood_adjustments[item.key])}`).join('、')}，讓外在表現更有層次。`
      : '血型這一層提供的是細微修飾，讓原本的人格骨架在行為表現上更穩定。');

  const individuality_summary = result.ai_individuality_summary?.trim()
    || (nameEffects.length
      ? `姓名作為最後校正器，把個體差異集中深化在 ${nameEffects.map((item) => item.label).join('、')}，使整體人格輪廓更貼近你獨有的生命節奏。`
      : '姓名主要扮演最後校正器，讓整體人格模型更個人化，但不會推翻前面的天地結論。');

  const final_summary = result.ai_final_summary?.trim()
    || `三合一融合後，你最鮮明的特質集中在 ${topFinal.map((item) => item.label).join('、')}。這代表你在人格節奏上兼具先天底色、後天表現與個體差異，整體模型已經完整成形。`;

  const wealth_motivation_summary = `你的財富動機呈現 ${getTraitPhrase('wealth', result.final_scores.wealth)}，並且會受到 ${getTraitPhrase('execution', result.final_scores.execution)} 與 ${getTraitPhrase('security', result.final_scores.security)} 的共同影響。`;
  ...
  const wisdom_perspective = result.ai_wisdom_perspective?.trim()
    || `天地人塑造了你的傾向，但選擇塑造了你的人生。當你願意以善為本、保持誠信、持續修心時，這些人格能量會更穩定地朝好的方向展開。`;
...
export function enrichPreview(
  ...
  const preview_scores = Object.fromEntries(
    DIMENSION_META.map((item) => [item.key, clampPercentage(base_scores[item.key] + blood_adjustments[item.key])]),
  ) as DimensionScores;
  ...
  const skeleton_summary = aiSkeletonSummary?.trim()
    || `生日先建立人格骨架，目前最明顯的底層特徵是 ${topBase.map((item) => item.label).join('、')}。`;

  const behavior_summary = aiBehaviorSummary?.trim()
    || (bloodEffects.length
  ...
  const preview_summary = aiPreviewSummary?.trim()
    || `天地預分析已建立完成，目前最突出的能量集中在 ${topPreview.map((item) => item.label).join('、')}；接下來若加入姓名與性別，模型會進一步做個體化校正。`;
