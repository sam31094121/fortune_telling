...
### Port 永遠是 8888，天宿命理固定專用

```
專案目錄：C:\Users\DRAGON\Desktop\命理
啟動指令：npm run dev
本機網址：http://localhost:8888（天宿命理固定專用）
```

### 啟動前必做：先清舊 process

`npm run dev` 遇到 port 佔用會自動跳到 3001、3002，**這是錯的**。
原因：上一次的 server 沒有被關掉，殭屍 process 還佔著 8888。

**正確流程（PowerShell）：**
```powershell
# Step 1：清掉佔用天宿命理固定埠 8888 的 process
8888 | ForEach-Object {
    $port = $_
...
# Step 3：等 15 秒，確認 8888 上線
Start-Sleep -Seconds 15
netstat -ano | Select-String ":8888" | Select-String "LISTENING"
```
...
3. 重新啟動 dev server
4. 驗證：`Invoke-WebRequest http://localhost:8888/_next/static/css/...` 要回傳 200 且 > 5000 bytes
...
- 本機開發：http://localhost:8888（天宿命理固定專用，永遠是這個）
