...
// 老師合盤、命宮塔羅、年齡由後端算好送來；這裡只拿顯示用的字典與型別。
import {
  ZIWEI_DESTINY_ELEMENT_LABELS,
  ZIWEI_FOURTEEN_MAJOR_STAR_MATERIAL,
  ZIWEI_PROFESSIONAL_SUPPORT_STARS,
  ZIWEI_PROFESSIONAL_TRANSFORMATIONS,
  ZIWEI_TWELVE_PALACE_ORDER,
  createZiweiFallbackPalace,
  getZiweiAnnualSignal,
  normalizeZiweiPalaceName,
  type DestinyTarotMatch,
  type ZiweiTeacherSynthesis,
  type ZiweiTeacherTarotSlot,
} from '@/lib/ziwei-teacher-synthesis';
...
interface InsightResult {
  ...
  /** 後端算好的十二宮老師合盤（/api/insight-analyze）。 */
...
function maskedTeacherName(name?: string) {
  const clean = name?.trim() ?? '';
  return clean ? clean.slice(-2) : '你';
}

const ZIWEI_PALACE_PLAIN_TOPICS: Record<string, string> = {
  命宮: '你自己、你的性格、界線與人生選擇',
  身宮: '你自己在現實中的行動方式與承擔',
  兄弟宮: '手足、同儕與共同責任',
  夫妻宮: '親密關係、承諾與相處界線',
...
function plainPalaceTopic(palaceName: string) {
...
/**
 * 紫微的恐怖與鬼魅不是兩套互相矛盾的結論。
 * LIFE_MASTER 提供命盤壓力與現實脈絡；NARRATIVE_MASTER 只把同一批星曜證據轉成象徵畫面。
 */
function ZiweiHorrorGhostMovieView({
  life,
  narrative,
  subjectName,
  subjectAge,
  ...
  return (
    <section className="mt-4 relative overflow-hidden rounded-[24px] border border-rose-300/35 bg-[radial-gradient(circle_at_82%_6%,rgba(190,24,93,0.25),transparent_31%),linear-gradient(145deg,rgba(69,10,10,0.6),rgba(46,16,101,0.42),rgba(2,6,23,0.86))] p-4 shadow-[0_0_48px_rgba(190,24,93,0.14)]">
      ...
      <div className="relative mt-4 space-y-3 text-base font-semibold leading-7 text-white/80">
        <section className="relative overflow-hidden rounded-2xl border border-white/10 bg-black/25 p-4">
          ...
          <p className="mt-2">先從「{topic}」留下的舊痕跡開始：{life.pastPattern}</p>
          {narrative && <p className="mt-2 text-violet-100/85">鬼魅迴聲：{narrative.pastEcho}</p>}
        </section>
        <section className="relative overflow-hidden rounded-2xl border border-rose-200/25 bg-rose-950/30 p-4 shadow-[inset_0_0_28px_rgba(127,29,29,0.18)]">
          ...
          <p className="text-xs font-black tracking-[0.14em] text-rose-100">第二幕・警報逼近</p>
          <p className="mt-2">這一幕只談「{topic}」：{life.fearScene}</p>
          <p className="mt-2"><span className="font-black text-rose-200">重複模式：</span>{life.repeatedPattern}</p>
          <p className="mt-2"><span className="font-black text-rose-200">盲點：</span>{life.blindSpot}</p>
        </section>
...
function ZiweiTeacherTarotBridgePanel({ cards }: { cards: ZiweiTeacherTarotSlot[] }) {
  ...
  return (
    <section className="mt-4 rounded-2xl border border-amber-300/25 bg-[radial-gradient(circle_at_top,rgba(251,191,36,0.13),rgba(15,23,42,0.86)_58%,rgba(2,6,23,0.96)_100%)] p-4 sm:p-5">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-[11px] font-black uppercase tracking-[0.22em] text-amber-200">命宮塔羅 · 四宮對照</p>
...
function ZiweiTeacherSynthesisPanel({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setTimeRefresh((value) => value + 1), delay);
  ...
  return (
    <div className="mt-4 rounded-2xl border border-purple-300/25 bg-[linear-gradient(160deg,rgba(88,28,135,0.28),rgba(15,23,42,0.9)_60%,rgba(2,6,23,0.96))] p-4 sm:p-5">
      ...
      <p className="mt-4 text-[11px] font-semibold leading-5 text-white/40">老師判讀只根據你的正式命盤；命盤裡找不到證據的，不會寫成結論。</p>
...
function ZiweiDestinyCardView({
  ...
  subjectAge?: number | null;
  destinyTarot?: DestinyTarotMatch | null;
  teacherSynthesis?: ZiweiTeacherSynthesis | null;
  analysis?: InsightResult['ziweiSanFang'];
  annual?: InsightResult['annualFortune'];
  analysisId?: string;
  ...
  const majorPattern = analysis?.pattern?.name?.trim() || card.title;
  ...
  return (
    <section className="fortune-card overflow-hidden p-4 sm:p-7" aria-label="紫微神秘命宮卡">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,420px)_1fr] lg:items-start">
        <button
          type="button"
          onClick={() => setFlipped((value) => !value)}
          aria-pressed={flipped}
          data-print-keep
          className="group mx-auto block w-full max-w-[400px] text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-200/80"
        >
          <div className="[perspective:1400px]">
            ...
            >
              {/* 正面：命工卡 */}
              <div className="absolute inset-0 overflow-hidden rounded-[34px] border-2 border-amber-100/65 bg-[radial-gradient(circle_at_50%_13%,rgba(253,230,138,0.30),transparent_22%),radial-gradient(circle_at_50%_64%,rgba(180,83,9,0.18),transparent_42%),linear-gradient(160deg,rgba(30,16,8,0.98),rgba(10,12,26,0.99))] p-6 shadow-[0_30px_82px_rgba(0,0,0,0.52),0_0_48px_rgba(251,191,36,0.14)] [backface-visibility:hidden]">
                <div className="pointer-events-none absolute inset-3 rounded-[28px] border border-amber-100/25" />
                <div className="pointer-events-none absolute inset-5 rounded-[24px] border border-white/10" />
                <div className="pointer-events-none absolute inset-x-10 top-10 h-px bg-gradient-to-r from-transparent via-amber-100 to-transparent" />
                <div className="pointer-events-none absolute inset-x-10 bottom-10 h-px bg-gradient-to-r from-transparent via-amber-100/65 to-transparent" />
                <div className="relative flex h-full flex-col">
                  <div>
                    <div>
                      <p className="font-serif text-2xl font-black tracking-[0.08em] text-amber-50 sm:text-3xl">{subjectName || '你的紫微命盤'}</p>
                      <p className="mt-1 text-[11px] font-black tracking-[0.28em] text-amber-100/85">命盤大格局</p>
                    </div>
                  </div>
                  ...
                  <div className="my-auto text-center">
                    ...
                    <div className="mt-6 border-t border-amber-100/25 pt-4 text-amber-100/80">
                      ...
                      {card.heroStars.length ? card.heroStars.map((star) => (
                        <span key={star.name} className="inline-flex min-h-14 items-center rounded-full border-2 border-amber-100/70 bg-[radial-gradient(circle_at_32%_28%,rgba(253,230,138,0.46),rgba(180,83,9,0.32),rgba(69,26,3,0.40))] px-7 font-serif text-3xl font-black tracking-[0.1em] text-amber-50 shadow-[0_0_28px_rgba(251,191,36,0.28)]">
                          {star.name}
                        </span>
                      )) : <span className="text-sm font-black">主星待確認</span>}
                      </div>
                    </div>
                  </div>
              ...
              >
                <div className="relative flex h-full flex-col">
                  ...
                  <div className="mt-3 text-center">
                    <h3 className="break-words font-serif text-2xl font-black leading-tight text-amber-50">{destinyTarot?.cardNameZh ?? '命宮塔羅'}</h3>
                  </div>
                  <p className="mt-2 rounded-xl border border-white/10 bg-black/25 px-3 py-2 text-[11px] font-semibold leading-5 text-[color:var(--text-sub)]">{destinyTarot?.reason ?? '這份是更新前保存的結果，按「重新分析」就能翻出你的命宮塔羅牌。'}</p>
...
function ZiweiTwelvePalaceCards({
  ...
  if (analysis.timeConfidence !== 'exact') {
    return (
      <section className="fortune-card p-5 sm:p-7">
        <p className="text-xs font-black uppercase tracking-[0.28em] text-amber-300">時辰檢查</p>
  ...
  const sortedPalaces = ZIWEI_TWELVE_PALACE_ORDER.map((key) => palaceMap.get(key) ?? (createZiweiFallbackPalace(key) as ZiweiFullPalace));
  const teacherSynthesis = teacherPalaceKey && palaceMap.has(teacherPalaceKey) ? teacherSynthesisByPalace?.[teacherPalaceKey] ?? null : null;
...
export default function InsightPage() {
  ...
  */
  ...
  /*
    「匯出報告」＝ window.print()。收合的 <details> 內容瀏覽器根本不排版，
    CSS 攤不開，印出來只剩一排空框（2026-09-15 實測）。列印前全部打開，印完還原。
  */
  useEffect(() => {
    let reopened: HTMLDetailsElement[] = [];
    const openAll = () => {
      reopened = [...document.querySelectorAll<HTMLDetailsElement>('details:not([open])')];
      reopened.forEach((item) => { item.open = true; });
    };
    const restore = () => {
      reopened.forEach((item) => { item.open = false; });
      reopened = [];
    };
  ...
  return (
    <div className="app-bg min-h-screen overflow-hidden">
      ...
      <main ref={mainRef} className="relative z-10 mx-auto max-w-5xl px-4 pb-10 pt-14 sm:px-6 sm:pb-12 sm:pt-12 lg:pb-14 lg:pt-12">
        <Link
          href="/"
          className="feature-home-link feature-home-link--cyan mb-4 inline-flex w-fit"
          aria-label="返回首頁"
        >
          {"\u8fd4\u56de\u9996\u9801"}
        </Link>
        {!result ? (
          ...
          <div className="space-y-6 relative overflow-hidden">
            ...
            <ZiweiDestinyCardView card={result?.destinyCard} subjectName={input.name} subjectAge={result?.subjectAge} destinyTarot={result?.destinyTarot} teacherSynthesis={result?.teacherSynthesisByPalace?.MING ?? null} analysis={result?.ziweiSanFang} analysisId={result?.presentation?.analysisId ?? result?.analysisId} fiveElement={result?.fiveElement} />
            ...
            <ZiweiTwelvePalaceCards
              ...
              subjectAge={result?.subjectAge}
              teacherSynthesisByPalace={result?.teacherSynthesisByPalace}
            />
