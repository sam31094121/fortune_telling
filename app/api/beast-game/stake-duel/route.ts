...
export async function POST(request: Request) {
  try {
    ...
    if(!Array.isArray(entries)||entries.length!==5||new Set(entries.map(e=>e?.id)).size!==5||entries.some(e=>!e||typeof e.id!=='string'||!e.id.trim()||e.id.length>200||typeof e.cardId!=='string'||!getCard(e.cardId)))throw new Error('必須押五張有效收藏紀錄，戰鬥試用牌不能當押注。');
    ...
    return NextResponse.json({ok:true,rounds:match.round-1,stake:{...stake,netChange:stake.verdict==='LOST'?-5:stake.verdict==='WON'?1:0,selectedEntries:entries,forfeitedEntryIds:stake.verdict==='LOST'?entries.map(e=>e.id):[],message:stake.verdict==='LOST'?'押入的五張已輸掉':stake.verdict==='WON'?'原五張保留，額外獎勵一張':'五張原卡退回'}});
  } catch (error) {
