...
請輸出 420 到 650 字繁體中文，不要標題、不要條列。先以「${facts.shortName}，你現在 ${facts.age}」開場。姓名只能使用提供的後兩字，不能寫全名。接著依序用三個清楚標籤寫「以前的你・殘影：」「${facts.shortName}，你現在 ${facts.age}・警報：」「未來的你・門縫：」。
...
最後另起一段「鬼魅回應：」，直接對讀者說話，像門外的低語回應前面三段八字線索。低語要神祕、恐怖、有餘韻，但最後回到人的選擇與行動。再另起一段「五元素封印：」，只能使用指定寶物，說明收下寶物或直接靠自律執行，兩條路其實是同一件今天就能做的小事。
姓名：${facts.shortName}
目前年齡：${facts.age}
