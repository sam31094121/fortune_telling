  ...
  try {
    return await Promise.race([
      ...
      new Promise<T>((_, reject) => {
        timer = setTimeout(() => reject(new Error('Google 解盤逾時')), ms);
      }),
    ]);
  } finally {
...
export async function POST(request: Request) {
  try {
    ...
    if (!facts.dayMaster || !facts.structure) {
      return NextResponse.json({ ok: false, message: '命盤核心資料不足，暫不送 Google 解盤。' }, { status: 400 });
    }
    ...
    const response = await withTimeout(
      ai.models.generateContent({
        ...
        contents: `你是「Google 八字解盤老師」。只能根據下列已鎖定的客戶八字資料，用繁體中文產出可直接給客戶閱讀的正式解盤。
規則：不得重新排盤、不得改變用神或忌神、不得預言或保證未來、不得把戲劇化內容當事實。名字只可用後兩字。
請直接輸出 360 到 560 字的繁體中文解盤。先以「${facts.shortName}，你現在 ${facts.age}」開場，目標是把下方專業八字資料翻成客戶看得懂的白話文。
...
在最後另起一段「五元素寶物：」，說明以下寶物是這張命盤五行補強的遊戲任務，並把它翻成一項能練習的能力與今天就能開始的小行動。要清楚寫出兩條路線：收下寶物，是用看得見的遊戲任務降低開始行動的門檻；不收下寶物，也能靠自律與執行力直接做同一項練習。兩條路的核心都是今天願意開始，而不是等到十全十美；今天的一次練習，能先讓明天的行動不同。不得把寶物說成會保證成功、改變命運、帶來超自然效果，也不得用羞愧、威脅或貶低方式逼迫客戶收下。最後一句給一項可執行且不強迫的建議。
重要：本次「五元素寶物」是 Google 與恐怖鬼魅共用的唯一行動補強元素。所有今天可做的練習、補強方向與寶物說明，都必須只以提供的寶物元素為準，不可另外指定另一個元素當行動目標。用神只可作為命盤結構的背景依據，不可寫成另一條需要客戶再補的路線。
...
`,
    ...
    if (!reading || reading.length < 180 || chineseCharacters < 120) {
      throw new Error('Google 回覆過短，未達可顯示的解盤品質。');
    }

    return NextResponse.json({ ok: true, provider: 'Google Gemini', reading }, { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Google 八字解盤暫時無法完成。';
