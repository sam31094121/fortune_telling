...
async function recordSupabaseDirectFallback(supabase: NonNullable<ReturnType<typeof getVisitorSupabaseClient>>) {
  ...
  if (error) {
    console.error('[ai-like] direct fallback read failed', error.message);
  }

  const localCount = await readLocalCountFloor();
  const remoteCount = normalizeCount(data?.total_count);
  const totalCount = Math.max(localCount, remoteCount, AI_LIKE_INITIAL_COUNT) + 1;
...
export async function POST(request: Request) {
  ...
  if (!supabase) {
    try {
      const result = await recordLocalAiLike(eventId, ipHash);
      return NextResponse.json(
  ...
  if (error || !row) {
    ...
    try {
      ...
      try {
        ...
        console.error('[ai-like] direct fallback failed', directFallbackError);
        return NextResponse.json(
