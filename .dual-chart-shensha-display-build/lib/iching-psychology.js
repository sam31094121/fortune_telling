...
exports.patternNameOf = patternNameOf;
exports.buildGhostDecoding = buildGhostDecoding;
exports.formatGhostDecoding = formatGhostDecoding;
exports.buildEmpathicReading = buildEmpathicReading;
exports.buildEmpathicFromHexagram = buildEmpathicFromHexagram;
exports.formatEmpathicReading = formatEmpathicReading;
exports.buildConciseOnion = buildConciseOnion;
const iching_engine_1 = require("./iching-engine");
/** 八卦 → 心理側寫：name=卦、persona=外在呈現（上卦用）、inner=內在自我（下卦用）、term=對應心理學專有名詞 */
const TRIGRAM_PSYCHOLOGY = {
    乾: {
...
function buildGhostDecoding(hexagram) {
...
function formatGhostDecoding(hexagram) {
 ...
 * 改在畫面上請客戶補時辰。
 */
function buildEmpathicReading(name, birthDate, shichenIndex) {
    return buildEmpathicFromHexagram(name, (0, iching_engine_1.castHexagramFromBirth)(birthDate, shichenIndex));
}
/**
 * 引擎共用入口：任何模組已起好的卦（八字老師、姓名學、配對）直接傳進來，
 * 共感層與該模組的卦象保證同一卦、同一口徑。
 */
function buildEmpathicFromHexagram(name, hexagram) {
 ...
 */
function formatEmpathicReading(reading) {
    return [
        reading.greeting,
 ...
 * 要客戶三秒看懂時用這一版。
 */
function buildConciseOnion(hexagram) {
    const upper = TRIGRAM_PSYCHOLOGY[hexagram.upper.name];
    const lower = TRIGRAM_PSYCHOLOGY[hexagram.lower.name];
    const mind = CHANGING_LINE_MIND[hexagram.changingLine] ?? CHANGING_LINE_MIND[2];
    return [
