...
exports.ZIWEI_MALEFIC_STAR_NAMES = exports.ZIWEI_AUSPICIOUS_STAR_NAMES = void 0;
exports.calculateZiweiSanFang = calculateZiweiSanFang;
const true_solar_time_1 = require("./true-solar-time");
const shichen_engine_1 = require("./shichen-engine");
const chartEngine_1 = require("./ziwei/chartEngine");
...
exports.ZIWEI_AUSPICIOUS_STAR_NAMES = new Set([
    '左輔', '右弼', '文昌', '文曲', '天魁', '天鉞', '祿存', '天馬', '恩光', '天貴', '台輔', '封誥', '龍池', '鳳閣', '紅鸞', '天喜', '三台', '八座',
]);
exports.ZIWEI_MALEFIC_STAR_NAMES = new Set([
    '擎羊', '陀羅', '火星', '鈴星', '地空', '地劫', '天空', '天刑', '天姚', '陰煞', '劫煞', '災煞', '大耗', '小耗', '白虎', '喪門', '吊客', '官符',
]);
function classifyZiweiSupportStars(stars) {
    const auspiciousStars = [];
    const maleficStars = [];
    const neutralStars = [];
    stars.forEach((star) => {
        if (exports.ZIWEI_AUSPICIOUS_STAR_NAMES.has(star))
            auspiciousStars.push(star);
        else if (exports.ZIWEI_MALEFIC_STAR_NAMES.has(star))
            maleficStars.push(star);
        else
...
function buildFullPalaceFocus(key, name) {
...
function resolveShichen(value, isTimeConfirmed) {
    if (typeof value === 'number' && value >= 0 && value <= 12) {
        return { index: value, confidence: isTimeConfirmed === false ? 'estimated' : 'exact' };
    }
...
function parseBirthTime(value) {
    ...
    if (!match)
        throw new Error('出生時間格式錯誤，請使用 HH:mm。');
...
function transformationLabel(value) {
    if (typeof value === 'string')
        return value || null;
    if (!value?.key)
        return null;
    const keyMap = { A: '祿', B: '權', C: '科', D: '忌' };
    return keyMap[value.key] ?? value.name ?? null;
}
function getDayMasterRelation(dayMasterElement, starElement) {
    if (starElement === dayMasterElement)
        return '比和';
    if (GENERATES[starElement] === dayMasterElement)
        return '生扶日主';
    if (GENERATES[dayMasterElement] === starElement)
        return '日主洩秀';
    if (CONTROLS[dayMasterElement] === starElement)
        return '日主制化';
    if (CONTROLS[starElement] === dayMasterElement)
        return '制約日主';
    return '五行待校';
}
function makeCrossCheck(palace, dayMasterElement, elementBalance) {
...
function identifyCorePattern(palaces) {
    ...
    const hasEvery = (stars) => stars.every((star) => coreStars.includes(star));
    const hasDistributedStars = (stars) => {
    ...
    const sharesPalace = (stars) => corePalaces.some((palace) => stars.every((star) => palace.majorStars.includes(star)));
...
function calculatePatternMetrics(palaces, pattern, dayMasterElement) {
    const corePalaces = palaces.filter((palace) => palace.key !== 'QIAN_YI');
    const coreStars = [...new Set(corePalaces.flatMap((palace) => palace.majorStars))];
    ...
    const trinePalaceCoverage = pattern.stars.length === 0
        ? 0
        : Math.round((corePalaces.filter((palace) => palace.majorStars.some((star) => patternStarSet.has(star))).length /
    ...
    for (const star of coreStars) {
        ...
        if (relation === '比和' || relation === '生扶日主')
            supportiveRelationCount += 1;
        if (relation === '制約日主')
            constrainingRelationCount += 1;
    }
    return {
...
function calculateConsistencyScore(patternMetrics, pattern, dataCompleteness) {
    ...
    return Math.max(0, Math.min(100, Math.round(coreStructure * 0.2
...
function evidenceForPalace(palace, pattern, dataCompleteness) {
...
function buildTwelvePalacePrecisionAnalyses(allPalaces, pattern, dataCompleteness, timeConfidence) {
    const confidence = {
        birthDataCompleteness: dataCompleteness >= 85 ? 'high' : dataCompleteness >= 65 ? 'medium' : 'low',
        ruleCoverage: timeConfidence === 'exact' ? 'complete' : 'partial',
        signalConsistency: pattern.stars.length >= 3 ? 'concentrated' : 'mixed',
    };
    return TWELVE_PALACE_V4_KEYS.map((key) => {
        ...
        const evidence = palace
            ...
            : [{ sourceType: 'target_palace', sourceName: copy.name, effect: 'mixed', explanation: '目前命盤資料未回傳此宮位，前端只顯示保守提醒。' }];
        const eventEvidence = evidence.slice(0, Math.min(3, evidence.length));
        const priority = eventEvidence.length >= 3 ? 'P1' : 'P2';
        return {
            palaceKey: key,
...
function buildSanFangSummary(palaces, pattern) {
...
function calculateZiweiSanFang(input, existingCore) {
    ...
    const resolvedShichen = (0, shichen_engine_1.getShichenInfo)(timeIndex === 12 ? 0 : timeIndex);
    ...
    const calculationDate = hasLongitude
        ? (0, true_solar_time_1.calculateTrueSolarTime)(solarDate, input.longitude, 8)
        : solarDate;
    const chart = (0, chartEngine_1.generateZiweiChart)({
        calendarType: 'solar',
        date: input.birthDate,
        gender: input.gender,
        timeIndex,
    });
    (0, chartEngine_1.assertChartCertifiedForAi)(chart, { isTimeKnown: true, birthDate: input.birthDate, gender: input.gender });
    /*
    ...
    */
    if (existingCore)
        (0, bazi_reuse_1.assertReusableExactBaziCore)({
            gender: input.gender,
    ...
    const baziCore = existingCore ?? (0, engine_1.createBaziCore)({
        gender: input.gender,
        birthDate: `${calculationDate.getFullYear()}-${calculationDate.getMonth() + 1}-${calculationDate.getDate()}`,
        calendarType: 'SOLAR',
        birthTimeKnown: true,
        traditionalHour: engine_1.BRANCHES[(timeIndex === 12 ? 0 : timeIndex) % 12],
    ...
    const palaces = PALACE_CONFIG.map((config) => {
        ...
        if (!palace)
            throw new Error(`排盤缺少${config.name}。`);
        const transformations = palace.majorStars
            .map((star) => transformationLabel(star.mutagen))
            .filter((value) => Boolean(value));
        const minorStars = palace.minorStars.map((star) => star.name);
